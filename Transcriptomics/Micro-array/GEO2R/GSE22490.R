...
gset <- getGEO("GSE22490", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gs <- factor(sml)
groups <- make.names(c("Controls","RPL"))
levels(gs) <- groups
gset$group <- gs
...
par(mar=c(7,4,2,1))
title <- paste ("GSE22490", "/", annotation(gset), sep ="")
boxplot(ex[,ord], boxwex=0.6, notch=T, main=title, outline=FALSE, las=2, col=gs[ord])
legend("topleft", groups, fill=palette(), bty="n")

# expression value distribution
par(mar=c(4,4,2,1))
title <- paste ("GSE22490", "/", annotation(gset), " value distribution", sep ="")
...
plotSA(fit2, main="Mean variance trend, GSE22490")
