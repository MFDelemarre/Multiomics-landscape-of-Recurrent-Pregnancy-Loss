...
path <- paste(urld, "acc=GSE113790", "file=GSE113790_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
gs <- factor(sml)
groups <- make.names(c("control","cases"))
levels(gs) <- groups
...
plotDispEsts(ds, main="GSE113790 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE113790 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE113790", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
